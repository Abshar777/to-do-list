...
function sideBar() {
  ...
  return (
    <div className='w-2/12 h-full py-4 px-2  hidden md:flex flex-col border-r-[1px] border-zinc-800'>
      <Button icon={'ri-triangle-fill'} border={true} secIcon={"ri-expand-up-down-line"} name={'Abshar'} />
      <div className="md:flex flex-col gap-[.2rem] py-4">
        {navItems.map((e,i)=>(
           <Button key={i} icon={e.icon} border={e.border} count={e.count} secIcon={e.secIcon} name={e.name} />
        ))}
      </div>
